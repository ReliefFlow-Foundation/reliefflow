...
#[contract]
pub struct FraudChallengeContract;

#[contractimpl]
impl FraudChallengeContract {
...
mod test {
    ...
    fn test_initialize() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register_contract(None, FraudChallengeContract);
        let client = FraudChallengeContractClient::new(&env, &contract_id);
